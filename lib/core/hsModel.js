  ...
  _createClass(HsModel, [{
    ...
    value: function find(options) {
      return HsModel.HsAdapter.getSdos(HsModel.HsSchema.props.oId, HsModel.HsSchema.props.id, options).then(function (response) {
        return response.body !== undefined && response.body.length > 0 ? response.body.map(function (sdo) {
    ...
    value: function findOne(where) {
      ...
      }).then(function (modelOrModels) {
        return modelOrModels[0] !== undefined ? modelOrModels[0] : false;
      });
    }
    /**
    ...
    value: function findOneAndUpdate(where, update) {
      ...
      return HsModel.findOne(where, opts).then(function (matchedModel) {
        Object.assign(matchedModel, update.$set);
        return matchedModel.update().then(function (updatedModel) {
          return updatedModel;
        ...
        if (validated) {
          if (files === null) {
            ...
            return HsModel.HsAdapter.createSdoBlob(hsBlob.blobFormData).then(function (sdo) {
              return new HsModel(_objectSpread({}, propsForSdo, {
    ...
    key: "updateById",
    value: function updateById(sdoId, data) {
      data.md.r += 1;
      data.__v = data.md.r;
    ...
    key: "NOT_CONTAINS",
    get: function get() {
      return _hsConstants.NOT_CONTAINS;
    }
    /**
